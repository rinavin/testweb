export enum DateElement {
  YEAR = 'Y',
  MONTH = 'M',
  DAY = 'D',
  DOW = 'W'
}

export enum MenuEntryProgram_SrcContext {
  Main = 'M',
  Current = 'C'
}

export enum EditReturnCode {
  EDIT_CONTINUE,
  EDIT_AUTOSKIP,
  EDIT_RESTART
}

export enum GuiExpressionEvaluator_ControlsPersistencyClearOption {
  All = 1,
  CurrentForm
}

export enum DataModificationTypes {
  Update = 'u',
  Insert = 'i',
  Delete = 'd',
  None = ' '
}

export enum GuiMenuEntry_ImageFor {
  MENU_IMAGE_TOOLBAR,
  MENU_IMAGE_MENU,
  MENU_IMAGE_BOTH
}

export enum GuiMenuEntry_MenuType {
  MENU,
  PROGRAM,
  OSCOMMAND,
  SEPARATOR,
  SYSTEM_EVENT,
  INTERNAL_EVENT,
  USER_EVENT,
  WINDOW_MENU_ENTRY
}

export enum MgCheckState {
  UNCHECKED = 1,
  CHECKED,
  INDETERMINATE
}

export enum MarkMode {
  MARK_ALL_TEXT = 1,
  UNMARK_ALL_TEXT,
  MARK_SELECTION_TEXT
}

export enum MgCursors {
  ARROW = 1,
  WAITCURSOR,
  HAND,
  APPSTARTING,
  CROSS,
  HELP,
  IBEAM,
  NO,
  SIZEALL,
  SIZENESW,
  SIZENS,
  SIZENWSE,
  SIZEWE,
  UPARROW
}

/// <summary> modifier keys</summary>
export enum Modifiers {
  MODIFIER_ALT = 'A',
  MODIFIER_CTRL = 'C',
  MODIFIER_SHIFT = 'S',
  MODIFIER_NONE = ' ',
  MODIFIER_SHIFT_CTRL = 'X',
  MODIFIER_ALT_CTRL = 'Y',
  MODIFIER_ALT_SHIFT = 'Z'
}

export enum CommandType {
  OPEN_FORM,
  CREATE_FORM,
  INITIAL_FORM_LAYOUT,
  SHOW_FORM,
  CREATE_PLACEMENT_LAYOUT,
  EXECUTE_LAYOUT,
  SUSPEND_LAYOUT,
  RESUME_LAYOUT,
  CREATE_LABEL,
  CREATE_EDIT,
  CREATE_BUTTON,
  CREATE_COMBO_BOX,
  CREATE_LIST_BOX,
  CREATE_RADIO_CONTAINER,
  CREATE_RADIO_BUTTON,
  CREATE_IMAGE,
  CREATE_CHECK_BOX,
  CREATE_TAB,
  CREATE_TABLE,
  CREATE_COLUMN,
  CREATE_SUB_FORM,
  CREATE_BROWSER,
  CREATE_GROUP,
  CREATE_RICH_EDIT,
  CREATE_RICH_TEXT,
  CREATE_LINE,
  DISPOSE_OBJECT,
  CREATE_FRAME_FORM,
  CREATE_CONTAINER,
  PROP_SET_CONTROL_NAME,
  PROP_SET_TEXT,
  PROP_SET_READ_ONLY,
  PROP_SET_TEXT_SIZE_LIMIT,
  PROP_SET_ITEMS_LIST,
  PROP_SET_IMAGE_FILE_NAME,
  PROP_SET_IMAGE_DATA,
  PROP_SET_IMAGE_LIST,
  PROP_SET_DEFAULT_BUTTON,
  PROP_SET_FONT,
  PROP_SET_BACKGOUND_COLOR,
  PROP_SET_FOREGROUND_COLOR,
  PROP_SET_ALTENATING_COLOR,
  PROP_SET_COLOR_BY,
  PROP_SET_TOOLTIP,
  PROP_SET_VISIBLE,
  PROP_SET_ENABLE,
  PROP_SET_MENU_ENABLE,
  PROP_SET_VISIBLE_LINES,
  PROP_SET_WALLPAPER,
  PROP_SET_PLACEMENT,
  PROP_SET_BOUNDS,
  PROP_SET_MIN_WIDTH,
  PROP_SET_MIN_HEIGHT,
  PROP_SET_RIGHT_TO_LEFT,
  PROP_SET_CHECKED,
  PROP_SET_SELECTION,
  PROP_SET_LAYOUT_NUM_COLUMN,
  PROP_SET_LINE_VISIBLE,
  PROP_SET_RESIZABLE,
  PROP_SET_ROW_HEIGHT,
  PROP_SET_TITLE_HEIGHT,
  PROP_SET_BOTTOM_POSITION_INTERVAL,
  PROP_SET_ALLOW_REORDER,
  PROP_SET_SORTABLE_COLUMN,
  PROP_SET_RIGHT_BORDER,
  PROP_SET_TOP_BORDER,
  PROP_SET_COLUMN_PLACMENT,
  PROP_SET_ICON_FILE_NAME,
  SET_WINDOW_STATE,
  SET_COLUMN_ORG_WIDTH,
  SET_COLUMN_START_POS,
  SET_TABLE_ITEMS_COUNT,
  SET_TABLE_VIRTUAL_ITEMS_COUNT,
  SET_TABLE_VSCROLL_THUMB_POS,
  SET_TABLE_VSCROLL_PAGE_SIZE,
  SET_TABLE_INCLUDES_FIRST,
  SET_TABLE_INCLUDES_LAST,
  SET_TABLE_TOP_INDEX,
  SET_SELECTION_INDEX,
  INVALIDATE_TABLE,
  REFRESH_TABLE,
  REFRESH_TMP_EDITOR,
  UPDATE_TMP_EDITOR_INDEX,
  CREATE_TABLE_ROW,
  UNDO_CREATE_TABLE_ROW,
  SET_TABLE_ROW_VISIBILITY,
  VALIDATE_TABLE_ROW,
  INSERT_ROWS,
  REMOVE_ROWS,
  CLEAR_TABLE_COLUMNS_SORT_MARK,
  PROP_SET_URL,
  PROP_SET_ROW_HIGHLIGHT_BGCOLOR,
  PROP_SET_ROW_HIGHLIGHT_FGCOLOR,
  PROP_SET_INACTIVE_ROW_HIGHLIGHT_BGCOLOR,
  PROP_SET_INACTIVE_ROW_HIGHLIGHT_FGCOLOR,
  PROP_SET_ACTIVE_ROW_HIGHLIGHT_STATE,
  PROP_SET_SHOW_ELLIPSIS,
  PROP_SET_GRADIENT_COLOR,
  PROP_SET_GRADIENT_STYLE,
  SELECT_TEXT,
  MOVE_ABOVE,
  SET_FOCUS,
  WRITE_TO_MESSAGE_BOX,
  CLOSE_FORM,
  REMOVE_SUBFORM_CONTROLS,
  START_TIMER,
  STOP_TIMER,
  BEEP,
  PROP_SET_MENU,
  PROP_RESET_MENU,
  CREATE_MENU,
  REFRESH_MENU_ACTIONS,
  CREATE_MENU_ITEM,
  DELETE_MENU_ITEM,
  DELETE_MENU,
  CREATE_TOOLBAR,
  DELETE_TOOLBAR,
  CREATE_TOOLBAR_ITEM,
  DELETE_TOOLBAR_ITEM,
  CREATE_STATUS_BAR,
  PROP_SET_SB_PANE_WIDTH,
  CREATE_SB_LABEL,
  CREATE_SB_IMAGE,
  CREATE_TREE,
  CREATE_TREE_NODE,
  MOVE_TREE_NODE,
  SET_EXPANDED,
  SET_CHILDREN_RETRIEVED,
  DELETE_TREE_NODE,
  CREATE_FRAME_SET,
  PROP_HORIZONTAL_PLACEMENT,
  PROP_VERTICAL_PLACEMENT,
  SHOW_TMP_EDITOR,
  PROP_SET_TRANSLATOR,
  PROP_SET_HORIZANTAL_ALIGNMENT,
  PROP_SET_VERTICAL_ALIGNMENT,
  PROP_SET_MULTILINE_WORDWRAP_SCROLL,
  PROP_SET_MULTILINE_VERTICAL_SCROLL,
  PROP_SET_MULTILINE_ALLOW_CR,
  PROP_SET_MULTILINE,
  PROP_SET_PASSWORD_EDIT,
  PROP_SET_STYLE_3D,
  PROP_SET_CHECKBOX_MAIN_STYLE,
  PROP_SET_BORDER,
  PROP_SET_MINBOX,
  PROP_SET_MAXBOX,
  PROP_SET_SYSTEM_MENU,
  PROP_SET_TITLE_BAR,
  PROP_SET_FORM_BORDER_STYLE,
  ORDER_MG_SPLITTER_CONTAINER_CHILDREN,
  PROP_SHOW_FULL_ROW,
  PROP_HOT_TRACK,
  PROP_SHOW_BUTTONS,
  PROP_LINES_AT_ROOT,
  PROP_SHOW_LINES,
  PROP_SHOW_SCROLLBAR,
  PROP_COLUMN_DIVIDER,
  PROP_LINE_DIVIDER,
  PROP_ROW_HIGHLITING_STYLE,
  PROP_SET_LINE_STYLE,
  PROP_SET_LINE_WIDTH,
  PROP_SET_LINE_DIRECTION,
  SET_ENV_ACCESS_TEST,
  SET_ENV_TOOLTIP_TIMEOUT,
  SET_ENV_SPECIAL_TEXT_SIZE_FACTORING,
  SET_ENV_SPECIAL_FLAT_EDIT_ON_CLASSIC_THEME,
  PROP_TAB_CONTROL_SIDE,
  PROP_SET_STARTUP_POSITION,
  PROP_SET_RADIO_BUTTON_APPEARANCE,
  PROP_SET_THREE_STATE,
  PROP_SET_CHECK_BOX_CHECKED,
  PROP_SET_HOVERING_COLOR,
  PROP_SET_VISITED_COLOR,
  PROP_SET_EXPANDED_IMAGEIDX,
  PROP_SET_COLLAPSED_IMAGEIDX,
  PROP_SET_PARKED_IMAGEIDX,
  PROP_SET_PARKED_COLLAPSED_IMAGEIDX,
  PROP_SET_IMAGE_LIST_INDEXES,
  PROP_SET_TAB_SIZE_MODE,
  COMBO_DROP_DOWN,
  SET_ACTIVETE_KEYBOARD_LAYOUT,
  SET_TAG_DATA_LINK_VISITED,
  UPDATE_MENU_VISIBILITY,
  ALLOW_UPDATE,
  SET_ALIGNMENT,
  BULLET,
  INDENT,
  UNINDENT,
  CHANGE_COLOR,
  CHANGE_FONT,
  CHANGE_COLUMN_SORT_MARK,
  SET_CURRENT_CURSOR,
  SET_FRAMES_WIDTH,
  SET_FRAMES_HEIGHT,
  SET_WINDOWSTATE,
  REORDER_COLUMNS,
  RESTORE_COLUMNS,
  PROP_SET_ALLOW_DRAG,
  PROP_SET_ALLOW_DROP,
  SETDATA_FOR_DRAG,
  PERFORM_DRAGDROP,
  ACTIVATE_FORM,
  SUSPEND_PAINT,
  RESUME_PAINT,
  PROP_SET_SELECTION_MODE,
  LOCK_WINDOW_UPDATE,
  ENABLE_XPTHEMES,
  APPLY_CHILD_WINDOW_PLACEMENT,
  TOGGLE_ALTERNATE_COLOR_FOR_FIRST_ROW,
  REGISTER_DN_CTRL_VALUE_CHANGED_EVENT,
  PROP_SET_ROW_PLACEMENT,
  SET_TABLE_ORG_ROW_HEIGHT,
  SET_TOOLBAR,
  CREATE_ENTRY_IN_CONTROLS_MAP,
  REMOVE_ENTRY_FROM_CONTROLS_MAP,
  PROCESS_PRESS_EVENT,
  SET_MARKED_ITEM_STATE,
  ACCEPT_DVCONTROL_CHANGES,
  UPDATE_DVCONTROL_ROW,
  ADD_DVCONTROL_HANDLER,
  REMOVE_DVCONTROL_HANDLER,
  CREATE_ROW_IN_DVCONTROL,
  UPDATE_DVCONTROL_COLUMN,
  DELETE_DVCONTROL_ROW,
  SET_DVCONTROL_ROW_POSITION,
  REJECT_DVCONTROL_COLUMN_CHANGES,
  PROP_SET_FOCUS_COLOR,
  PROP_SET_TITLE_COLOR,
  SET_DESIGNER_VALUES,
  PROP_SET_TOP_BORDER_MARGIN,
  PROP_SET_DIVIDER_COLOR,
  PROP_SET_TITLE_FGCOLOR,
  PROP_SET_HOT_TRACK_COLOR,
  PROP_SET_HOT_TRACK_FGCOLOR,
  PROP_SET_COLUMN_FILTER,
  PROP_SET_FILL_WIDTH,
  PROP_SET_BORDER_STYLE,
  PROP_SET_TITLE_PADDING,
  PROP_SET_EDIT_HINT,
  PROP_SET_EDIT_HINT_COLOR,
  PROP_SET_MULTI_COLUMN_DISPLAY,
  SET_SHOULD_APPLY_PLACEMENT_TO_HIDDEN_COLUMNS,
  PROP_SET_SELECTED_TAB_COLOR,
  PROP_SET_SELECTED_TAB_FGCOLOR,
  SET_ENV_LAMGUAGE,
  PROP_SET_ROW_BG_COLOR,
  SET_LAST_IN_CONTEXT,
  PROP_SET_BORDER_COLOR,
  SET_CARET,
  SET_RECORDS_BEFORE_CURRENT_VIEW,
  SET_VALUE,
  SET_ATTRIBUTE,
  SET_PROPERTY,
  SET_CLASS,
  SET_STYLE,
  REFRESH_TASK,
  PROP_SET_USER_PROPERTY
}

export enum HtmlProperties {
  Visible = "visible",
  Enabled = "enabled",
  Text = "text",
  ItemsList ="itemslist",
  Image = "image",
  Tooltip = "tooltip",
  SelectedValue = "selectedvalue",
  ShowBorder="showborder",
  PlaceHolder="placeholder",
  Password="password",
  TabIndex="tabindex",
  SelectedRow="selectedRow"
}

export enum MenuStyle {
  MENU_STYLE_PULLDOWN = 1,
  MENU_STYLE_CONTEXT,
  MENU_STYLE_TOOLBAR
}

export enum DockingStyle {
  NONE,
  BOTTOM,
  FILL,
  LEFT,
  RIGHT,
  TOP
}

export enum RaisedBy {
  USER_ACTION,
  CLOSE_SYSTEM_MENU,
  CTRL_GOTO,
  MOUSE_WHEEL,
  INTERNAL,
  KBPUT
}

export enum TraverseMode {
  NONE = 1,
  WITHIN_CONTROL,
  BETWEEN_CONTROLS
}

export enum ClipFormats {
  FORMAT_USER,
  FORMAT_TEXT,
  FORMAT_OEM_TEXT,
  FORMAT_RICH_TEXT,
  FORMAT_HTML_TEXT,
  FORMAT_HYPERLINK_TEXT,
  FORMAT_DROP_FILES,
  FORMAT_UNICODE_TEXT,
  FORMAT_UNKNOWN = 'c'
}
